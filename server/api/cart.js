...
router.post("/:userId", requireToken, isUser, async (req, res, next) => {
  try {
    ...
    localCart.forEach(async (product) => {
      try {
        currentProduct = await Product.findByPk(product.productId)
        if (product.quantity > currentProduct.stock) return res.status(400).send({error: "stock"})
      } catch (err) {
        next(err)
      }
    })
...
//CHECKOUT
router.put('/:userId/checkout',requireToken, isUser, async (req, res, next) => {
  try {
    let data = []
    ...
    for (const cartitem of dbCart.cartitems) {
      ...
      if (cartitem.quantity > product.stock) return res.status(400).send({error: "stock"})
    }
